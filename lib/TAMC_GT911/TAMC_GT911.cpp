...
void TAMC_GT911::reset() {
  // LOCAL PATCH (galactic-unicorn-pad): on the CrowPanel 7" (WZ8048C070) the
  // GT911 INT/RST lines are not broken out, so they are passed as -1 (0xFF).
  // Driving those invalid pins spams the GPIO error log, and worse: when the
  // config read below fails on a momentarily busy I2C bus, setResolution()
  // writes a corrupted config back to the controller, which then NAKs every
  // poll (constant "i2cRead returned Error -1"). The panel already ships
  // pre-configured for 800x480, so skip the whole reset and just poll.
  if (pinInt == 0xFF || pinRst == 0xFF) return;
